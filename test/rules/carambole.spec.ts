...
describe("Carambole", () => {
  ...
  it("Carambole no point switch player", (done) => {
    ...
    container.table.outcome.push(Outcome.collision(container.table.balls[1], container.table.balls[2], 1))
    container.processEvents()
    expect(container.controller).to.be.an.instanceof(WatchAim)
    done()
  })

  it("Carambole score point transition to Aim", (done) => {
    container.controller = new PlayShot(container)
    container.isSinglePlayer = false
  ...
  it("Carambole no point single player switch ball", (done) => {
    container.controller = new PlayShot(container)
    container.table.cueball.setStationary()
    container.eventQueue.push(new StationaryEvent())
    container.table.outcome.push(Outcome.collision(container.table.balls[1], container.table.balls[2], 1))
  ...
  it("Carambole second player uses second cueball", (done) => {
    const rules = RuleFactory.create(rule, container)
    rules.secondToPlay()
  ...
  it("Valid carambole outcome", (done) => {
    const outcomes: Outcome[] = []
    outcomes.push(Outcome.collision(oppononetBall, cueBall, 1))
    outcomes.push(Outcome.collision(cueBall, redBall, 1))
    expect(Outcome.isCarambolePoint(cueBall, outcomes)).to.be.true
    done()
  })

  it("Invalid carambole outcome", (done) => {
    const outcomes: Outcome[] = []
    outcomes.push(Outcome.collision(cueBall, oppononetBall, 1))
    expect(Outcome.isCarambolePoint(cueBall, outcomes)).to.be.false
    done()
  })
})
